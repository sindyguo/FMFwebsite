...
export default {
  ...
  props: {
    ...
    explain: {
      type: String,
      default: 'Slide to the right to complete the verification'
    },
    imgSize: {
      type: Object,
      default() {
  ...
  methods: {
    ...
    // 鼠标松开
      ...
      if (this.status && this.isEnd === false) {
        ...
        this.$api.captchaCheck(data).then(res => {
          if (res.repCode === '0000') {
            ...
            this.passFlag = true
            this.tipWords = `${((this.endMovetime - this.startMoveTime) / 1000).toFixed(2)}s verify success`
    ...
    getPictrue() {
      ...
      this.$api.captchaGet(data).then(res => {
        if (res.repCode === '0000') {
