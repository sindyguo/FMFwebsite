  ...
  <div class="MyUploadImages">
    <div class="vcontainer main-container">
      <div class="hcontainer vcenter flex-between">
        ...
        <div v-if="step == 1" class="order-back" @click="handleBack"></div>
      </div>
      ...
      <div class="fill vcontainer MyUploadImages-content" v-if="step === 1">
        ...
        <div class="MyUploadImages-content-form">
          <div class="MyUploadImages-content-form-title">Select three images of nuchal translucency showing the time and date and the calliper placement.</div>
      ...
      <div class="fill vcontainer MyUploadImages-content" v-else>
        ...
        <div class="action-buttons">
          <el-button type="primary" size="small" @click="handleBack">Own Page</el-button>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    handleUpload() {
      let flag = this.fileList.every(item => item.assessment || item.url)
      if(!flag) {
    ...
    async createImageReviewLogFn() {
      let data = {
        ...this.form,
        uploadImageList: this.fileList.map(item=> item.id).join(","),
      ...
      this.beforeUploadFn(file, this.uploadObj, async (file)=>{
        ...
        try {
          ...
          let data = {  
            ...
            name: file.name,
            url: res.url,
            assessment: this.fileList[index].assessment
          }
    ...
    async getImageDetailFn() {
      try {
        ...
        if(res.code === 0) {
          ...
          this.fileList = Array.from({ length: this.form.imageUploadLimit }).map(() => ({ name: "", url: "", id: "", assessment: "" }))
        }
      } catch (error) {
